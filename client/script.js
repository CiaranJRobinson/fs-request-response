...
const ohMy = () => {
    ...
    .then(response => {
        for(i=0;i<response.data.length;i++){
            ...
            newEle.textContent = response.data[i];
...
const repeatMyParam = () => {
    ...
    .then(response => {
        let repeatTxt = document.getElementById("repeat-button");
        repeatTxt.textContent = response.data;})
}
...
// CODE HERE
function newFunc () {
    axios.get('http://localhost:3000/query-test?query=lions')
    .then(response =>{
